...
class SlidingUpPanel extends StatefulWidget {
  ...
  /// by default the Panel is open and must be swiped closed by the user.
  ///
  final double Function() maxPoint;
    ...
    if (widget.panel != null) {
      return GestureDetector(
        onVerticalDragUpdate: (DragUpdateDetails dets) {
          if (dets.delta.dy <= 0.0 && _panelPosition >= widget.maxPoint()) return;
          _onGestureSlide(dets.delta.dy);
        },
        onVerticalDragEnd: (DragEndDetails dets) {
          if (dets.velocity.pixelsPerSecond.dy <= 0 && _panelPosition >= widget.maxPoint()) return;
  ...
  void _onGestureEnd(Velocity v) {
    ...
    if (widget.panelSnapping) {
      if (minDistance == d2Close) {
        ...
        _animatePanelToPosition(widget.maxPoint(), duration: Duration(milliseconds: 400), curve: Curves.linearToEaseOut);
      }
    }
  }
